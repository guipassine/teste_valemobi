...
$id = $_GET['id'];
$name;
$sql = "SELECT * FROM tipos_de_negocios WHERE tipo_negocio_id={$id}";
$result = $conn->query($sql);
if($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        $name = $row['tipo_negocio_nome'];
        $id = $row['tipo_negocio_id'];
    }
}
$conn->close();
if(!$name && !$id) {
...
<html>
    ...
    <body>
        <main id="interface">
            ...
            <div id="content">
                <h4>EDITAR TIPO DE NEGÓCIO</h4>
                <br />
                <form role="form" action="update_tipo_negocio.php?" method="post">
                    
                    <input type="hidden" id="id" name="id" value="<?php echo $id; ?>" />
                    
                    <div class="form-group">
                        <label for="tipo_negocio">Tipo Negócio</label>
                        <input value="<?php echo $name; ?>" class="form-control" id="tipo_negocio" name="tipo_negocio" placeholder="Digite aqui..." />
                    </div>
